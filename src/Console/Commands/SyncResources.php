...
namespace Opscale\NovaAPI\Console\Commands;
...
final class SyncResources extends Command
{
     ...
     * @var string
     */
    protected $signature = 'nova-api:sync-resources
     ...
     * @var string
     */
    protected $description = 'Sync Nova resources to nova-api config file';
    ...
    private function processResources(): void
    {
        ...
        $this->info('Successfully synced ' . count($filteredResources) . ' resources to config/nova-api.php');
    ...
    {
        $configPath = config_path('nova-api.php');

        if (! File::exists($configPath)) {
            $this->error('Config file not found at: ' . $configPath);
            $this->info('Please publish the config file first: php artisan vendor:publish --tag=nova-api-config');
